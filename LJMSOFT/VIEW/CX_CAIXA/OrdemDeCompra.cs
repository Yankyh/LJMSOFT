...
namespace LJMSOFT.VIEW.CX_CAIXA
{
    public partial class OrdemDeCompra : Form
    {
        ...
        private void fornecedorDropDown(object sender, EventArgs e)
        {
            conexao.Conectar();
            //Limpa a combo box
        ...
        private void recalcularValor(object sender, EventArgs e)
        {
            ...
            else
            {
                if (checkBox.Checked)
                {
                    ...
                    try
                    {
                        valor = Convert.ToInt32(valorTotalBox.Text.Replace(',', '.').Replace('R', ' ').Replace('$', ' '));
                    }
                    catch (FormatException b)
                    {
                        MessageBox.Show("Informe somente números inteiros neste campo");
                    }
                    try
                    {
                        quantidade = Convert.ToInt32(quantidadeBox.Text);
                    }
                    catch (FormatException b)
                    {
                        MessageBox.Show("Informe somente números inteiros neste campo");
                            ...
                            if (tipoPagamentoCombo.Text != "")
                            {
                                if (itensCombo.Text != "")
                                {
                                    if (quantidadeBox.Text != "")
                                    {
                                        ...
                                        {
                                            MessageBox.Show("A data de previsão é menor que a data do pedido");
                                        }
                                    }
                                    else
                                    {
